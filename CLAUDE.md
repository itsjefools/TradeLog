...
1. **ダーク/ライトモード切り替え** — システム設定連動オプションあり
2. **本人確認バッジ** — プロフィールに表示
3. **国籍選択（国旗付き）** — `profiles` テーブルに `nationality` カラム追加、プロフィール画面に表示
4. **トレードスタイル** — 新規登録時に選択 + プロフィール設定でいつでも編集可能。プロフィール画面に表示（他ユーザーからも見える）
5. **パスワード強化** — 8文字以上 + 記号か数字を最低1つ必須
6. **ボトムタブを5つに変更** — フィード / 記録 / 分析 / **スクール** / プロフィール
7. **通貨ペア選択を検索式に変更** — 入力で即フィルタリング、対応ペア大幅増量